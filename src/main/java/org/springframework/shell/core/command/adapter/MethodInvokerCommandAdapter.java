...
public class MethodInvokerCommandAdapter extends AbstractCommand {
    ...
    public ExitStatus execute(CommandContext commandContext) throws Exception {
        var result = super.execute(commandContext);
        commandContext.outputWriter().flush();
        return result;
    }
    ...
    private List<Object> prepareArguments(CommandContext commandContext) {
        ...
        for (int i = 0; i < parameters.length; i++) {
            ...
            @io.github.hider.mongoway.Workaround
            var remainingArgumentsAnnotation = parameters[i].getAnnotation(io.github.hider.mongoway.commands.RemainingArguments.class);
            if (remainingArgumentsAnnotation != null) {
                var rawValues = commandContext.parsedInput()
                    .arguments()
                    .stream()
                    .skip(processedArguments)
                    .map(CommandArgument::value)
                    .toList();
                var value = this.conversionService.convert(rawValues, parameterTypes[i]);
                args.add(value);
            }
        }
        return args;
